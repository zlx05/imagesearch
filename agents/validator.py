...
@lru_cache(maxsize=1)
def _safe_clip_device() -> str:
    try:
        _, _, _, _, device = get_clip_runtime()
        return device
    except Exception:
...
def watermark_ocr_enabled() -> bool:
    return env_flag("VALIDATOR_ENABLE_WATERMARK_OCR", True) and bool(watermark_ocr_provider_chain())
...
def watermark_ocr_provider_chain() -> List[str]:
    ...
    for provider in providers:
        provider = normalize_ocr_provider_name(provider)
        if provider in {"paddleocr_vl", "baidu", "rapidocr"} and provider not in normalized:
            normalized.append(provider)
    # Always append rapidocr as ultimate fallback (unless it's already the only provider)
    if "rapidocr" not in normalized:
        normalized.append("rapidocr")
    return normalized
...
def get_paddleocr_vl_runtime() -> Any:
    ...
    if not device:
        try:
            import paddle; device = "gpu" if paddle.is_compiled_with_cuda() else "cpu"
        except Exception:
            device = "cpu"
...
def validate_node(state: AgentState) -> AgentState:
    ...
    for index, node in enumerate(input_nodes, start=1):
        ...
        if progress_callback:
            event_node = {
                "id": updated_node.get("id") or node.get("id") or index,
                "idx": index,
                "title": updated_node.get("title") or updated_node.get("description") or node.get("title") or "",
                "url": updated_node.get("url") or updated_node.get("page_url") or node.get("url") or "",
                "image_url": updated_node.get("image_url") or updated_node.get("thumbnail_url") or node.get("image_url") or "",
                "thumbnail_url": updated_node.get("thumbnail_url") or updated_node.get("image_url") or node.get("thumbnail_url") or "",
                "platform": updated_node.get("platform") or updated_node.get("engine") or updated_node.get("source") or "",
                "engine": updated_node.get("engine") or node.get("engine") or "",
                "source": updated_node.get("source") or node.get("source") or "",
                "similarity": updated_node.get("similarity"),
                "validation_decision_type": updated_node.get("validation_decision_type") or "",
            }
            progress_callback(
                "validate_progress",
                index,
                ...
                {
                    "index": index - 1,
                    "decision": "keep" if is_similar else "reject",
                    "node": event_node,
                    "keep_count": len(validated_nodes),
                    "reject_count": rejected_count,
                    "similarity": updated_node.get("similarity"),
                    "reason": updated_node.get("validation_reason") or updated_node.get("reason") or "",
                },
                progress_log,
            )
    ...
    try:
        ...
        def _extract_node(n: Dict[str, Any]) -> Dict[str, Any]:
            sigs = n.get("validation_signals") if isinstance(n.get("validation_signals"), dict) else {}
            evidence = n.get("validator_evidence") if isinstance(n.get("validator_evidence"), dict) else {}
            topo = n.get("topology_hints") if isinstance(n.get("topology_hints"), list) else []
            tamper_sigs = n.get("tampering_signals") if isinstance(n.get("tampering_signals"), list) else (
                evidence.get("tampering_signals") or sigs.get("tampering_signals") or []
            )
            return {
                "id": n.get("id"),
                "title": n.get("title") or "",
                "url": n.get("url") or n.get("page_url") or "",
                "image_url": n.get("image_url") or "",
                "thumbnail_url": n.get("thumbnail_url") or n.get("image_url") or "",
                "author": n.get("author") or n.get("publisher") or "",
                "publisher": n.get("publisher") or n.get("author") or "",
                "published_at": n.get("published_at") or "",
                "platform": n.get("platform") or n.get("platform_family") or "",
                "platform_family": n.get("platform_family") or "",
                ...
                "is_similar": n.get("is_similar"),
                "reason": n.get("reason") or n.get("validation_reason") or "",
                "image_variant": n.get("image_variant") or "",
                "validation_reason": n.get("validation_reason") or n.get("reason") or "",
                "validation_decision_type": n.get("validation_decision_type") or "",
                "validation_decision_stage": n.get("validation_decision_stage") or "",
                # Tampering
                "suspected_tampering": n.get("suspected_tampering"),
                "tampering_signals": tamper_sigs,
                "tampering_reason": n.get("tampering_reason") or "",
                "tampering_type": n.get("tampering_type") or "",
                # Topology
                "topology_hints": topo,
                # Validator evidence (nested)
                "validator_evidence": {
                    "decision_type": evidence.get("decision_type") or n.get("validation_decision_type") or "",
                    "decision_stage": evidence.get("decision_stage") or n.get("validation_decision_stage") or "",
                    "similarity": evidence.get("similarity") or n.get("similarity"),
                    "visual_score": evidence.get("visual_score") or sigs.get("visual_score") or n.get("visual_score"),
                    "text_score": evidence.get("text_score") or sigs.get("text_score") or n.get("text_score"),
                    "phash_similarity": evidence.get("phash_similarity") or sigs.get("phash_similarity") or n.get("phash_similarity"),
                    "clip_similarity": evidence.get("clip_similarity") or sigs.get("clip_similarity") or n.get("clip_similarity"),
                    "ocr_content_overlap": evidence.get("ocr_content_overlap") or sigs.get("ocr_text_overlap") or sigs.get("ocr_overlap"),
                    "retrieved_text_overlap": evidence.get("retrieved_text_overlap") or sigs.get("retrieved_text_overlap"),
                    "image_variant": evidence.get("image_variant") or n.get("image_variant") or "",
                    "suspected_tampering": evidence.get("suspected_tampering") or n.get("suspected_tampering"),
                    "tampering_signals": tamper_sigs,
                    "tampering_type": evidence.get("tampering_type") or n.get("tampering_type") or "",
                    "topology_hints": topo,
                    # Watermark
                    "watermark_detected": bool(sigs.get("watermark_ocr_used") or evidence.get("watermark_detected")),
                    "watermark_platforms": evidence.get("watermark_platforms") or sigs.get("watermark_platforms") or [],
                    "watermark_accounts": evidence.get("watermark_accounts") or sigs.get("watermark_accounts") or {},
                    "watermark_text": evidence.get("watermark_text") or sigs.get("watermark_text") or [],
                    "watermark_source": evidence.get("watermark_source") or sigs.get("watermark_source") or "none",
                    "watermark_ocr_used": bool(sigs.get("watermark_ocr_used") or evidence.get("watermark_ocr_used")),
                    "candidate_content_text": evidence.get("candidate_content_text") or sigs.get("candidate_content_text") or "",
                    "ocr_skipped": bool(sigs.get("ocr_skipped") or evidence.get("ocr_skipped")),
                    "ocr_skip_reason": evidence.get("ocr_skip_reason") or sigs.get("ocr_skip_reason") or "",
                },
                # Watermark top-level
                "watermark_detected": bool(sigs.get("watermark_ocr_used")),
                "watermarks": sigs.get("watermarks") or sigs.get("target_watermarks") or [],
                "watermark_platforms": evidence.get("watermark_platforms") or sigs.get("watermark_platforms") or [],
                "watermark_accounts": evidence.get("watermark_accounts") or sigs.get("watermark_accounts") or {},
                "watermark_text": evidence.get("watermark_text") or sigs.get("watermark_text") or [],
                "watermark_evidence": {
                    "source": sigs.get("watermark_source") or "none",
                    "provider_chain": sigs.get("watermark_ocr_provider_chain") or [],
                    "error": sigs.get("watermark_ocr_error") or "",
                },
        ...
        result_payload = {
            "target_image": {
                "local_path": state.get("target_image", {}).get("local_path") or "",
                "filename": state.get("target_image", {}).get("filename") or "",
            },
            "summary": {
                ...
                "device": _safe_clip_device(),
            },
            "nodes_data": [_extract_node(n) for n in deduplicated_nodes],
        }
